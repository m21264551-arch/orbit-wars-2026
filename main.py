# main.py — v6.1 "Shuming+Ajay hybrid, anti-snipe off"
# Foundation: public shuming_exp30 (4870 lines, the strongest public agent).
# Tweaks layered on top:
#   - SEARCH_DEPTH2_ENABLED = True (ajay's counter-snipe penalty)
#   - SEARCH_MAX_ACTIONS_TO_PICK_2P kept at 9 (shuming's wider beam)
#   - MULTIPRONG_ENABLED = False (it hurt in 2p against ajay/shuming-class agents)
#   - _depth2_penalty optimized: forward_project hoisted out of the per-enemy
#     loop (ajay12's caching optimization — same answer, cheaper)
#   - ANTI_SNIPE_ENABLED default flipped False (v6.1) — disabling the
#     anti-snipe target penalty gains +2 wins / 50 vs shuming on two
#     held-out seed ranges and is neutral vs lb1039/ajay (30 games each).
#   - v6.2 explored 4P broader search + higher VALUE_WEIGHT_4P + prod-sort
#     candidate ordering. Validated as noise across 160 fresh-seed 4P games
#     (+1 net win across seeds 700000 and 800000). All v6.2 levers remain
#     accessible via V62_*/V126_VALUE_WEIGHT_4P env vars but defaults are
#     reverted to v6.1. The V62_4P_PAYLOAD_PRESSURE / V62_4P_COMPOUND
#     pressure experiments stay gated off after local smoke showed
#     overcommit.
#   - 2026-06-04 also tested SEARCH_DEPTH2_ENABLED for 4P and
#     SEARCH_DISABLES_CHEAP_PICKUP=False on fresh seeds 700/800/900k.
#     Cheap-pickup 4P re-enable: -2 wins / 80 on seed 700k, reverted.
#     Depth-2 4P: +3/+2/-4 across three seed ranges → +1 net / 240 games,
#     noise. V62_SEARCH_DEPTH2_4P and V63_DISABLE_CP_4P env vars stay wired
#     but defaults remain v6.1 behavior. See notes/v6_param_tuning_notes.md
#     "2026-06-04 depth-2 / cheap-pickup ablation" section.
#
# Many TUNE_*/V12{4,8}_* env vars are exposed for further experiments; all
# defaults match v5 hybrid except V124_ANTI_SNIPE (now "0").
#
# Source: shuming_exp30 + ajay12 notebooks + measured ablations.
...
SEARCH_MAX_PER_SOURCE_4P = int(os.environ.get("V62_SEARCH_MAX_PER_SOURCE_4P", "3"))
SEARCH_MAX_ACTIONS_TO_EVAL_4P = int(os.environ.get("V62_SEARCH_EVAL_4P", "12"))
SEARCH_MAX_ACTIONS_TO_PICK = 5
SEARCH_MAX_ACTIONS_TO_PICK_4P = int(os.environ.get("V62_SEARCH_PICK_4P", "5"))
SEARCH_MAX_ACTIONS_TO_PICK_2P = 9
SEARCH_DISABLES_CHEAP_PICKUP = os.environ.get("V63_DISABLE_CP_4P", "1") != "0"
HAMMER_MELIS_VERIFY = True
SEARCH_DEPTH2_ENABLED = True
SEARCH_DEPTH2_4P_ENABLED = os.environ.get("V62_SEARCH_DEPTH2_4P", "0") != "0"
SEARCH_ORDER_4P_ENABLED = os.environ.get("V62_SEARCH_ORDER_4P", "0") != "0"
...
F1B_EXPAND_BONUS = float(os.environ.get("TUNE_EXPAND_BONUS", "3.0"))
...
SO1_STATIC_BONUS_2P = float(os.environ.get("TUNE_STATIC_BONUS_2P", "2.179862"))
...
VALUE_WEIGHT_2P = float(os.environ.get("TUNE_VALUE_WEIGHT_2P", "4.86118"))
VALUE_WEIGHT_4P = float(os.environ.get("V126_VALUE_WEIGHT_4P", "2.0"))


FOUR_P_PAYLOAD_PRESSURE_ENABLED = os.environ.get("V62_4P_PAYLOAD_PRESSURE", "0") != "0"
FOUR_P_COMPOUND_ENABLED = os.environ.get("V62_4P_COMPOUND", "0") != "0"
FOUR_P_PRESSURE_START = int(os.environ.get("V62_4P_PRESSURE_START", "18"))
FOUR_P_PRESSURE_END = int(os.environ.get("V62_4P_PRESSURE_END", "125"))
FOUR_P_PRESSURE_MIN_AVAIL = int(os.environ.get("V62_4P_PRESSURE_MIN_AVAIL", "18"))
FOUR_P_PRESSURE_MIN_PROD = int(os.environ.get("V62_4P_PRESSURE_MIN_PROD", "3"))
FOUR_P_PRESSURE_MAX_TRAVEL = int(os.environ.get("V62_4P_PRESSURE_MAX_TRAVEL", "28"))
FOUR_P_PRESSURE_MAX_PER_TURN = int(os.environ.get("V62_4P_PRESSURE_MAX_PER_TURN", "4"))
FOUR_P_PRESSURE_RATIO_PROD2 = float(os.environ.get("V62_4P_RATIO_PROD2", "0.45"))
FOUR_P_PRESSURE_RATIO_PROD3 = float(os.environ.get("V62_4P_RATIO_PROD3", "0.55"))
FOUR_P_PRESSURE_RATIO_PROD4 = float(os.environ.get("V62_4P_RATIO_PROD4", "0.65"))
FOUR_P_PRESSURE_RATIO_ENEMY = float(os.environ.get("V62_4P_RATIO_ENEMY", "0.70"))

# V65 Producer-style planner — gated by V65_PRODUCER_4P. Default off.
# See notes/producer_planner_scope.md for Phase A/B/C plan and A/B contract.
# The frozen v6.1 baseline lives at baselines/main_v61_baseline.py.
PRODUCER_4P_ENABLED = os.environ.get("V65_PRODUCER_4P", "0") != "0"
PRODUCER_4P_TURN_MIN = int(os.environ.get("V65_PRODUCER_TURN_MIN", "18"))
PRODUCER_4P_TURN_MAX = int(os.environ.get("V65_PRODUCER_TURN_MAX", "200"))
PRODUCER_4P_MAX_WAVES_PER_TURN = int(os.environ.get("V65_PRODUCER_MAX_WAVES", "2"))
PRODUCER_4P_MAX_TRAVEL = int(os.environ.get("V65_PRODUCER_MAX_TRAVEL", "30"))
PRODUCER_4P_MIN_AVAIL = int(os.environ.get("V65_PRODUCER_MIN_AVAIL", "15"))
PRODUCER_4P_DISCOUNT = float(os.environ.get("V65_PRODUCER_DISCOUNT", "0.97"))
PRODUCER_4P_HORIZON = int(os.environ.get("V65_PRODUCER_HORIZON", "200"))
PRODUCER_4P_OVERKILL = float(os.environ.get("V65_PRODUCER_OVERKILL", "1.20"))
PRODUCER_4P_MIN_SCORE = float(os.environ.get("V65_PRODUCER_MIN_SCORE", "30.0"))
PRODUCER_4P_MIN_PROD_SHARE = float(os.environ.get("V65_PRODUCER_MIN_PROD_SHARE", "0.25"))
...
# V64 instrumentation — env-gated, no behavior change. When V64_INSTRUMENT=1,
# the agent aggregates mode_log activations + launch volume and writes one
# JSON-Lines record per checkpoint to the path in V64_INSTR_FILE (default
# "v64_instr.jsonl"). Append-mode; safe to share across games in one run.
SUBSYSTEM_INSTRUMENT_ENABLED = os.environ.get("V64_INSTRUMENT", "0") != "0"
SUBSYSTEM_INSTRUMENT_FILE = os.environ.get("V64_INSTR_FILE", "v64_instr.jsonl")
_instr_counts = None
_instr_actions = 0
_instr_ships = 0
_instr_game_id = 0
...
HAMMER_ENABLED = True
HAMMER_STOCKPILE_MIN = int(os.environ.get("TUNE_HAMMER_STOCKPILE_MIN", "50"))
HAMMER_TARGET_PROD_MIN = 2
HAMMER_PROD_SHARE_TRIGGER = float(os.environ.get("TUNE_HAMMER_PROD_SHARE", "0.40"))
HAMMER_OVERKILL_RATIO = float(os.environ.get("TUNE_HAMMER_OVERKILL", "1.30"))
...
BRAIN_LEAD_RESERVE_MIN_SHIPS = int(os.environ.get("V64_BRAIN_LEAD_MIN_4P", "200"))
...
MODE_PARAMS_2P = {
    ...
    "pressure": {
        "expand_k_opening": int(os.environ.get("TUNE_2P_PRESS_EXPAND_K_OPEN", "5")),
        "expand_max_travel_opening": 35,
        "expand_k_mid": int(os.environ.get("TUNE_2P_PRESS_EXPAND_K_MID", "2")),
        "expand_max_travel_mid": 52,
        "hammer_prod_share": float(os.environ.get("TUNE_2P_PRESS_HAMMER_SHARE", "0.25")),
        "hammer_overkill": float(os.environ.get("TUNE_2P_PRESS_HAMMER_OK", "1.177645")),
        "hammer_stockpile_min": int(os.environ.get("TUNE_2P_PRESS_HAMMER_STOCKPILE", "25")),
    },
}
...
def _depth2_penalty(world, our_action, top_opp_actions=2):
    ...
    # ajay12 optimization: forward_project depends only on our_action+world,
    # not on the specific enemy planet we're evaluating. Compute once outside
    # the loop to save N-1 expensive sim runs per candidate action.
...
def search_step_action(world, max_per_source=3, max_actions_to_eval=10,
    ...
    if not actions:
        return []
    if not world.is_2p and SEARCH_ORDER_4P_ENABLED:
        def _four_p_action_key(act):
            owner = int(act.get("target_owner", -1))
            prod = int(act.get("target_prod", 0))
            enemy = owner not in (-1, world.player)
            return (
                -prod,
                0 if enemy else 1,
                int(act.get("arrival_turn", 999)),
                float(act.get("raw_dist", 999.0)),
            )
        actions.sort(key=_four_p_action_key)
    ...
    for src in world.my_planets:
        ...
        for raw, t in picks:
            ...
            actions.append({
                ...
                "ships": int(ships),
                "raw_dist": float(raw),
                "target_owner": int(t.owner),
                "target_prod": int(t.production),
            })
    return actions
...
def _four_p_pressure_send_count(world, src, tgt, preferred, max_avail):
    """4P compounding: after the first captures, send enough payload to make
    high-production grabs arrive faster and survive the immediate scrum.
    """
    if not FOUR_P_PAYLOAD_PRESSURE_ENABLED or world.is_2p:
        return preferred
    if world.step < FOUR_P_PRESSURE_START or world.step > FOUR_P_PRESSURE_END:
        return preferred
    if max_avail < FOUR_P_PRESSURE_MIN_AVAIL:
        return preferred
    prod = max(0, int(tgt.production))
    is_enemy = tgt.owner not in (-1, world.player)
    if not is_enemy and prod < 2:
        return preferred

    if is_enemy:
        ratio = FOUR_P_PRESSURE_RATIO_ENEMY
    elif prod >= 4:
        ratio = FOUR_P_PRESSURE_RATIO_PROD4
    elif prod >= 3:
        ratio = FOUR_P_PRESSURE_RATIO_PROD3
    else:
        ratio = FOUR_P_PRESSURE_RATIO_PROD2

    pressure_ships = int(max_avail * ratio)
    return max(preferred, min(max_avail, pressure_ships))
...
def plan_solo_capture(world, src, tgt, max_avail, max_travel):
    ...
    preferred = _four_p_pressure_send_count(world, src, tgt, preferred, max_avail)
...
def _handle_search_expand_4p(world, available, spent, target_locked, moves, mode_log):
    ...
    """
    if world.is_2p:
        max_per_source = SEARCH_MAX_PER_SOURCE
        max_actions_to_eval = 12
        pick_limit = SEARCH_MAX_ACTIONS_TO_PICK
        use_depth2 = SEARCH_DEPTH2_ENABLED
    else:
        max_per_source = SEARCH_MAX_PER_SOURCE_4P
        max_actions_to_eval = SEARCH_MAX_ACTIONS_TO_EVAL_4P
        pick_limit = SEARCH_MAX_ACTIONS_TO_PICK_4P
        use_depth2 = SEARCH_DEPTH2_ENABLED and SEARCH_DEPTH2_4P_ENABLED
    actions = search_step_action(
        world, max_per_source=max_per_source,
        max_actions_to_eval=max_actions_to_eval,
        use_depth2=use_depth2,
    )
    committed_sources = set()
    committed_targets = set()
    for act in actions[:pick_limit * 2]:
        if act["score"] <= 0:
            continue
        ...
        if len(committed_sources) >= pick_limit:
            break
...
def handle_four_p_compound_pressure(world, available, spent, target_locked, moves, mode_log):
    """4P-only midgame throughput pass.

    The top replay gap is not the first neutral; it is turns 25-75. This pass
    lets idle sources keep converting surplus into high-production captures or
    enemy pressure after the normal search/expand pass has taken its shot.
    """
    if not FOUR_P_COMPOUND_ENABLED or world.is_2p:
        return
    if world.step < FOUR_P_PRESSURE_START or world.step > FOUR_P_PRESSURE_END:
        return
    if LAUNCH_BLACKOUT_ENABLED and world.step >= TOTAL_STEPS - LAUNCH_BLACKOUT_TURNS:
    ...
    max_travel = max(
        FOUR_P_PRESSURE_MAX_TRAVEL,
        int(world.mode_params.get("expand_max_travel_mid", EXPAND_MAX_TRAVEL_MID)),
    )
    sources = sorted(
        world.my_planets,
    ...
    fired = 0
    for src in sources:
        if fired >= FOUR_P_PRESSURE_MAX_PER_TURN:
            break
        if mode_log.get(src.id):
            continue
        avail = _routine_avail(world, src, available[src.id] - spent[src.id])
        if avail < FOUR_P_PRESSURE_MIN_AVAIL:
            continue
        ...
        for tgt in world.planets:
            ...
            prod = max(0, int(tgt.production))
            is_enemy = tgt.owner not in (-1, world.player)
            if not is_enemy and prod < FOUR_P_PRESSURE_MIN_PROD:
                continue
            if is_enemy and prod < 2 and not world.is_late:
                continue
            ...
            if tgt.owner == -1:
                if RACE_ENABLED:
                    ...
                    if enemy_eta is not None and turns > enemy_eta:
                        continue
                if not _capture_holds_against_snipe(world, tgt, turns, int(ships)):
                    continue
                if not _endgame_roi_ok(world, tgt, int(ships), turns):
                    continue
                if not _neutral_tempo_ok(world, tgt, int(ships), turns):
            ...
            score = prod * 24.0 - float(turns) * 2.0 - float(ships) * 0.20 - raw * 0.08
            if is_enemy:
                score += 18.0
            if world.contest_leader and tgt.owner == world.leader_id:
                score += 10.0
            if tgt.id in _enemy_recently_launched:
                score += 8.0
            if tgt.id in _freshly_lost_planets:
                score += 10.0

            key = (score, prod, -turns, -raw)
            payload = (tgt, angle, turns, ships)
        ...
        if best is None:
            continue
        _key, payload = best
        tgt, angle, turns, ships = payload
        _commit_fleet(world, moves, spent, target_locked,
                      src.id, tgt.id, angle, turns, int(ships))
        mode_log[src.id] = "compound-pressure"
        fired += 1


def _producer_target_pv(tgt, world):
    """Discounted present-value of holding target for the horizon, net of
    nearby enemy threat. Closed-form geometric sum of production discounted
    by V65_PRODUCER_DISCOUNT for V65_PRODUCER_HORIZON turns (or game end).
    """
    prod = max(0, int(tgt.production))
    if prod <= 0:
        return 0.0
    horizon = min(PRODUCER_4P_HORIZON, max(1, TOTAL_STEPS - world.step))
    d = PRODUCER_4P_DISCOUNT
    if d >= 0.9995:
        pv = float(prod * horizon)
    else:
        pv = prod * (1.0 - d ** horizon) / (1.0 - d)
    danger = 0.0
    for e in world.enemy_planets:
        ed = dist(e.x, e.y, tgt.x, tgt.y)
        if ed < 25.0:
            danger += int(e.ships) * (1.0 - ed / 25.0)
    is_enemy = tgt.owner not in (-1, world.player)
    if is_enemy:
        pv *= 1.5
    return pv - danger * 0.5


def handle_producer_pressure_4p(world, available, spent, target_locked, moves, mode_log):
    """V65 Producer-style planner (Phase A) — target-first ROI greedy.

    For each non-owned target with prod >= 2:
      score = PV(production) - danger - enemy bonus
    Sort targets descending. For each picked target (up to MAX_WAVES_PER_TURN),
    find the single best idle source that can safely capture (closest by
    arrival turn, max surplus tiebreak). Reuses existing safety:
    plan_solo_capture, _capture_holds_against_snipe, _endgame_roi_ok,
    _neutral_tempo_ok, _routine_avail.

    4P-only via the early-return gate. Conservative Phase A: small K, high
    MIN_AVAIL, MIN_SCORE floor — meant to be additive on top of expand
    without overcommit. Phase B will add multi-source synchronized waves.
    """
    if not PRODUCER_4P_ENABLED or world.is_2p:
        return
    if world.step < PRODUCER_4P_TURN_MIN or world.step > PRODUCER_4P_TURN_MAX:
        return
    if LAUNCH_BLACKOUT_ENABLED and world.step >= TOTAL_STEPS - LAUNCH_BLACKOUT_TURNS:
        return
    # V65 v3 lead-only gate: Producer compounds the lead when we're already at
    # or above the 4P average production share, and stays out of the way when
    # we're behind (the tight strong-opponent games where v1/v2 lost wins).
    if world.my_prod_share < PRODUCER_4P_MIN_PROD_SHARE:
        return

    # Rank candidate targets by present-value score
    candidates = []
    for tgt in world.planets:
        if tgt.owner == world.player:
            continue
        ...
        if is_enemy and prod < 2 and not world.is_late:
            continue
        score = _producer_target_pv(tgt, world)
        if score < PRODUCER_4P_MIN_SCORE:
            continue
        candidates.append((score, tgt))
    if not candidates:
        return
    candidates.sort(key=lambda x: -x[0])
    candidates = candidates[: PRODUCER_4P_MAX_WAVES_PER_TURN * 4]

    fired = 0
    for score, tgt in candidates:
        if fired >= PRODUCER_4P_MAX_WAVES_PER_TURN:
            break
        if tgt.id in target_locked:
            continue
        best = None
        for src in world.my_planets:
            if mode_log.get(src.id):
                continue
            base_avail = available[src.id] - spent[src.id]
            avail = _routine_avail(world, src, base_avail)
            if avail < PRODUCER_4P_MIN_AVAIL:
                continue
            raw = dist(src.x, src.y, tgt.x, tgt.y)
            if raw / MAX_SPEED > PRODUCER_4P_MAX_TRAVEL + 4:
                continue
            plan = plan_solo_capture(world, src, tgt, avail, PRODUCER_4P_MAX_TRAVEL)
            if plan is None:
                continue
            angle, turns, ships = plan
            if tgt.owner == -1:
                ...
                if not _neutral_tempo_ok(world, tgt, int(ships), turns):
                    continue
            # Cap per-launch ship spend to OVERKILL margin × min_need
            need = effective_needed_to_capture(tgt, turns, world)
            cap_ships = int(max(ships, need * PRODUCER_4P_OVERKILL))
            cap_ships = min(cap_ships, avail)
            if cap_ships < ships:
                cap_ships = int(ships)  # never below safe plan
            # Prefer closer arrival, then larger surplus retained
            key = (-int(turns), int(avail) - int(cap_ships))
            if best is None or key > best[0]:
                best = (key, src, angle, int(turns), int(cap_ships))
        if best is None:
            continue
        _, src, angle, turns, ships = best
        _commit_fleet(world, moves, spent, target_locked,
                      src.id, tgt.id, angle, turns, int(ships))
        mode_log[src.id] = "producer-launch"
        fired += 1
...
def plan_moves(world, deadline=None):
    ...
    if not _over_budget():
        handle_four_p_compound_pressure(world, available, spent, target_locked, moves, mode_log)
    ...
    # V65 Producer runs AFTER the stockpile chain so mega-hammer / accumulator
    # claim their leads first. Producer picks up only truly-idle backline sources.
    if not _over_budget():
        handle_producer_pressure_4p(world, available, spent, target_locked, moves, mode_log)
    ...
    if SUBSYSTEM_INSTRUMENT_ENABLED:
        global _instr_counts, _instr_actions, _instr_ships
        if _instr_counts is None:
            _instr_counts = defaultdict(int)
        for tag in mode_log.values():
            _instr_counts[tag] += 1
        for m in moves:
            _instr_actions += 1
            try:
                _instr_ships += int(m[2])
            except (IndexError, TypeError, ValueError):
                pass
        if world.step in (25, 50, 75, 100, 150, 200, 300, 499):
            import json as _json
            try:
                with open(SUBSYSTEM_INSTRUMENT_FILE, "a") as _f:
                    _f.write(_json.dumps({
                        "game": int(_instr_game_id),
                        "step": int(world.step),
                        "player": int(world.player),
                        "mode": "2P" if world.is_2p else "4P",
                        "actions": int(_instr_actions),
                        "ships": int(_instr_ships),
                        "my_planets": len(world.my_planets),
                        "my_prod": int(sum(int(p.production) for p in world.my_planets)),
                        "tags": dict(_instr_counts),
                    }) + "\n")
            except OSError:
                pass
...
def agent(obs, config=None):
    ...
    global _instr_counts, _instr_actions, _instr_ships, _instr_game_id
